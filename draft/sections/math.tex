...
The localized section of fibers $\iota^*\tau: U \rightarrow \iota^*E$ is the sheaf $O(E)$. The neighborhood of points the sheaf lies over is the stalk $\mathscr{F}_k$ \cite{StalkSheaf2019}
...
where $A$ is composed of an asthethic mapping $\Xi = Q \circ \nu$ that builds the prerender render function $rho: S \rightarrow H$ functions described in section~\ref{sec:display}. $\XI$ builds $\rho$ functions on $S$ by compositing $\nu_{param}: c \mapsto \rho_{d \in D}$ functions that map data in some field $c$ to a subset of visual variables such as position or color\cite{bertinIIPropertiesGraphic2011, munznerChDataAbstraction} that are identified as parameters to the artist function. 

\begin{equation}
     \rho(u, v) = Q(\nu_param_{0}(\tau_{c_0}), \ldots \nu_param_{n}(\tau_{c_n}))(u,v)
    \label{eq:Q}
\end{equation}

$Q$ composites $\nu$ functions because multiple parameters can target the same field in $D$, for example $\nu_{ypos}$ and $\nu_{linewidth}$ together determine the y position of a line drawn on screen such that $Q: \nu_{ypos}, \nu_{linewidth} \rightarrow \rho_{Y}$. The artist is 
\begin{equation}
\begin{tikzcd}
    \iota^{*}E \arrow[rd, "\Xi" description] & H \arrow[d, "\rho^{-1}" description, bend right]                         \\
    U \arrow[u, "\tau" description]          & S \arrow[u, "\rho" description, bend right] \arrow[l, "\xi" description]
\end{tikzcd}
\end{equation}
