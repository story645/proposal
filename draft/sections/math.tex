...
\end{figure}
The scatter plot in \autoref{fig:math:artist:q:scatter} has a constant \textit{size} and color $\gsection_{RGB} = (0,0,0)$ that are defined as part of the point assembly function $\vmark(xpos, ypos)(\alpha, \beta)$.  
...
\begin{minipage}{.5\textwidth}
    \begin{align*}
        x &= size *\alpha \cos(\beta) + xpos \\[.5\baselineskip]
        y &= size *\alpha \sin(\beta) + ypos
    \end{align*}
\end{minipage}
\label{fig:math:artist:q:scatter:s}
\caption{lookup for scattter glyphs}
\end{figure}


The position of this swatch of color can be computed relative to the location on the disc \((\alpha, \beta) \in \gbase_{\dbasepoint}\) as shown in \autoref{fig:math:artist:q:scatter:s}. \note{EXPLAIN WHAT IS HAPPING IN THE FIGURE THAT GETS BACK TO gsection}

$\gsection(\gbasepoint) = (x, y, 0, 0, 0)$ colors the point (x,y) black. 
...
\end{figure}
In contrast, the line plot $\vmark(xpos, \hat{n}_{1}, ypos, \hat{n}_{2})(\alpha, \beta)$ in \autoref{fig:math:artist:q:line} has a \vindex\ function that is not only parameterized on \dbasepoint\ but also on the $\alpha$ distance along \dbasepoint\ and corresponding region in \gbase. 
...
\begin{figure} [H]
    ...
    \begin{minipage}{.5\textwidth}
        \begin{align*}
            ...
            \hat{n}_{1} = \frac{n_1}{\lvert n \rvert}, \,\hat{n}_{2} = \frac{n_2}{\lvert n \rvert} \\[.5\baselineskip]
            x &= xpos(\vindex(\alpha)) + width*\beta\hat{n}_1(\vindex(\alpha))  \\[.5\baselineskip]
            y &= ypos(\vindex(\alpha)) + width*\beta\hat{n}_2(\vindex(\alpha)) 
           \end{align*}
    \end{minipage}
    \label{fig:math:artist:q:line:s}
    \caption{lookup for line glyphs}
\end{figure}
