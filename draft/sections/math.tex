...
\begin{equation}
    d = (x, y, R, G, B)
\end{equation}
such that a graphic only has 2D position and color. To support overplotting and transparency, the fiber would be $D=R^{7}$ such that $d=(x,y,z, R, G, B, A)$. The location coordinates $x$ and $y$ are defined in terms of the display, while the \textrm{R,G,B} values are filled in via a lookup on $S$. 
...
We define a section of the displayed graphic as a region on D defined by the bounding box $b=\left[y_{top}, y_{bottom}, x_{right}, x_{left}\right]$
