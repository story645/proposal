...
\label{sec:data}
%%% 
We build on Butler's fiber bundle driven abstraction of data formats\cite{butlerVectorBundleClassesForm1992,butlerVisualizationModelBased1989},  ....


Fiber bundles are structures $(E,\,B,\,\pi ,\,F)$ where $E$, $F$, and $K$ are topological spaces that encode 
\begin{itemize}
...
where $E$ is the total space of data that $F$ lives in. The bundle is the projection map $\pi$
\begin{equation}
...
that binds the variable space $F$ to the base space $K$. 

%%probbaly need some more words here maybe, maybe pull that first picture back up now 


The fiber bundles mentioned in this work are assumed to be trivial\cite{spanier1989algebraic,LocallyTrivialFibre} unless otherwise specified. In the trivial bundle $E=K\times F$ such that the total space $E$ falls out and discussion can be limited to the $K$ and $F$ spaces. 


\subsubsection{Variables: Fiber Space $F$}
\label{sec:data_fiber}
The fiber is a topological space containing the set of all records that could possibly be in the dataset, even values that do not actually exist in the dataset. 

%%push straight math generalization up
%% We define w/ mapping, as Spivak

%%push down to an example, knock it back to semi concrete
...
\end{align}%the fiber is the set, variable is the tuple such that there is a map from the index i in F to the tuple (name, M_0)  for every U_{type} we can construct a map to the name and the type as described by Spivak 
