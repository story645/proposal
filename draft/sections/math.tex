...
As described in section~\ref{sec:display}, $rho: S \rightarrow H$ is composed of subfunctions that are dependent on the prerender fiber $D$. In the case of a screen where $D=\mathbb{R}^{7}$

\begin{equation}
    \rho (u, v) = (\rho_X(u, v), \rho_Y (u, v), \rho_Z (u, v), \rho_B (u, v), \rho_G (u, v), \rho_B (u, v), \rho_A (u, v))
\end{equation}

where $(u,v)$ are coordinates of a point on $S$. The map reduce function $Q: F\rightarrow \rho$ builds $\rho$ functions on $S$ by compositing $\nu_{param}: c \mapsto \rho_{d \in D}$ functions that map data in some field $c$ to a subset of visual variables such as position or color\cite{bertinIIPropertiesGraphic2011, munznerChDataAbstraction} that are identified as parameters to the artist function. 
