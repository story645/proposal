...
To support working with a subset of data, we can take the sheaf $\mathcal{O}(\dtotal)$ as input into the artist. This facilitates interactions such as zooming or updating the graphic based on zooming data. \dtotal\ restricted over a small enough neighborhood $U \subset \dbase$ is a locally trivial bundle over $U$\cite{LocallyTrivialFibre}. The sheaf $O(\dtotal)$ is the localized section of fibers $\iota^*\dsection: U \rightarrow \iota^*\dtotal$
