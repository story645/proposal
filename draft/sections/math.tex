...
The fiber is a topological space containing the set of all observations that could possible be in the dataset, even values that do not actually exist in the dataset. For example, a fiber $F$ that is the temperature variable from GHCN is the range of physically possible temperature values $\{tp_0, \ldots, tp_n\} \subset \mathbb{R}$. Lets rename this
\begin{equation}
F_0 = \{tp_0, \ldots, tp_n\} 
\end{equation}
so that more variables can be added to the fiber. Then lets add the time from GHCN $F_1 =\{t_0, \ldots, t_n\}$
\begin{equation}
F =  \{tp_0, \ldots, tp_n\}  \times \{ti_0, \ldots, ti_n\} 
\end{equation}
...
\begin{equation}
    F = \{tp_0, \ldots, tp_n\}  \times \{ti_0, \ldots, ti_n\} \times 
    \{(la_0, ln_0),\ldots, (la_j, ln_k), \ldots (la_n,ln_n)\}
\end{equation}
such that one record $r$ in the fiber would have the form $(tp, t, (la,ln))$. We also add the variable name and measurement scale $M$ monoid actions to the fiber $F_i$ such that 
\begin{align}
...
\subsubsection{Measurement Scale: Monoid Actions}
\label{sec:data_monoid}
The monoid actions $M_i$ is the set of transformations on $F_i$ that we expect to be preserved in the visual transform. For example, the temperature $F_0$ with units $\degree C$ 
Steven's measurement scales define group actions \cite{stevensTheoryScalesMeasurement1946, leaFormalizationMeasurementScale}
