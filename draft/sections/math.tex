...
%%use spivaks U w/ subset data {x \in R | x >= 0}
\begin{equation}
F_0 = \{T_0, \ldots, T_n\} 
\end{equation}
...
For example, the temperature $F_0$ defined in unit \textdegree C is on an interval scale; this means that the monoid actions $M_0$ on $F_0$ are the set of all transformations $tp_i + c$ such that %%generalize out to x in \U_{temp}, c in \R
%%the action of the interval is the + 
%% c is the element that shifts from x1 to x2
\begin{equation}
...
\begin{figure}
    ...
    \caption{Many non-trivial spaces can be made locally trivial by dividing $E$ into subspaces and defining transition functions between the edges on K for how to glue the two subspaces such that the sigmas are continuous.}
\end{figure}

We can represent some spaces that are not trivial as 
