...
\begin{equation}
    d = (x, y, R, G, B)
\end{equation}
such that a graphic only has 2D position and color. To support overplotting and transparency, the fiber would be $D=R^{7}$ such that $d=(x,y,z, R, G, B, A)$,
...
\end{equation}
 ...
 as the deformation retraction \cite{RetractionTopology2020} $\xi: S \rightarrow K$ that goes from a region $s \in S_{k}$ to its associated point $k$, such that when $\xi(s) = k$, $\xi^*\tau(s) = \tau(k)$. 
...
\end{figure}
Figure~\ref{fig:graphic_retraction_map} illustrates some examples of $\xi$. The base space $S$ is mapping to a in $D$. For example, in the scatter plot, $S$ encodes what the glyph looks like such that each piece of a single glyph maps to a different region on $S$. The line plot $S$ can encode position in the $\alpha$ dimension, but the $\beta$ dimension describes 

The  line plot $S$ encodes on a plane  by necessity needs to have a thickness to be visible; therefore we construct a rectangular base space $S$ that mirrors the plane on which we can encode the position in one dimension and thickness (should this pixel be colored?) in the other. The map $xi$ then goes from a rectangle to 
