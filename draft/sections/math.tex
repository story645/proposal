...
\textcolor{blue}{potentially more words on why fiber bundles stolen from like the scipy proposal but maybe this goes in the intro}
...
\begin{figure}[ht!]
    ...
    \caption{}.  
\end{figure}
In figure~\ref{fig:}, $\tau_1$ and $\tau_2$ encode two different timeseries of temperature. Both these timeseries live in the fiber $F$ which is an $\n \times \m$ array because it is the set of all possible (time, temperature) values. The base space $K$ encodes that all datasets in this fiberbundle are 1D continous.

\subsubsection{Variables: Fiber Space $F$}
\label{sec:data_fiber}
The fiber is a topological space that is the set of possible values of the data; the values themselves can be any dimension and type and have any continuity. We use Spivak's description of simplicial database schemas \cite{spivakSIMPLICIALDATABASES} as the basis of our fiber space because he binds the values in the fiber to variable names and types. Spivak constructs a set $U$ that is the disjoint union of all possible objects of the types $T\in DT$ that are in the datasets embedded in the fiber bundle. He then defines the single variable set $U_\sigma$ 
\begin{equation}
    \label{eq:data_types}
\begin{tikzcd}
...
\begin{equation}
F = U_{T_1}\times \ldots U{T_i} \ldots\timesU_{T_n}
\end{equation}
...
V_i = (c_i,\, T_i,\, U_{T_i}\)
\end{equation} 
we keep track of its name $c$, type $T$, and component in the fiber $U_{T}$. 
...
\begin{figure}[ht!]
    ...
    \caption{}}.
\end{figure}
For example, the data in figure~\ref{fig:fiber_example_plane} is a set of temperature measurements in \textdegree C and the times those measurements were taken at. Temperature values are floating point numbers that come from the set of real numbers $U_{float} = \mathbb{R}$ and times that are always positive $U_{datetime}= \mathbb{R}^+$, so the fiber is 
\begin{equation}
    F = \mathbb{R} \times \mathbb{R}^{+}
\end{equation} 
where $V_0 = (temperature,\, \texttt{float},\, \mathbb{R})$ and $V_1 = (time,\, \texttt{datetime},\, \mathbb{R}^+)$
In figure~\ref{fig:fiber_example_cube}, a third variable wind is added to the dataset. This wind variable is of type \texttt{wind} and has two components speed and direction $\{(m,d) \in \mathbb{R}^{2} \mid  0\leq m,\, 0 \leq d \leq 360\}$ . The new variable is added to the fiber
\begin{equation}
    F = \mathbb{R} \times \mathbb{R} \times \mathbb{R}^2
\end{equation} 
such that $V_2 = (wind,\, \texttt{wind},\, \mathbb{R}^{2})$  

\subsubsection{Measurement Scales: Monoid Actions}
\label{sec:data_monoid}
We formally describe the set of allowable operations in the fiber because a transformation on the data side needs to be reflected in the visual representation, discussed in more detail in section~\ref{sec:artist_nu}. We generalize this set of allowable transformations to the monoid actions $M_i$ on $F_i$. A monoid \cite{Monoid2021} $M_i$ is a set with an associative binary operator $\ast:M_i \times M_i\rightarrow M_i$. A monoid has an identity element $e\in M_i$ such that $e\ast a= a \ast e = a$ for all $a \in M_i$. A left monoid action \cite{SemigroupAction2021,ActionNLab} of $M_i$ is a set $U_{T_i}$ with an action $\bullet: M\times U_{T_i} \rightarrow U_{T_i}$ with the properties:
\begin{align*}
...
Steven's defined the measurement scales \cite{stevensTheoryScalesMeasurement1946,leaFormalizationMeasurementScale} in terms of the monoid actions permissible on the measurments; nominal data is permutable, ordinal data is monotonic, interval data is translatable, and ratio data is scalable \cite{weissteinSimilarityTransformation}. For example, given 
\begin{itemize}
    \item temperature $x \in \mathbb{R}$
    \item monoid addition operator $\ast = +$
    \item with monoid operations: $f: x\mapsto x + c_1 $, $g: x\mapsto x + c_2$
    \item monoid action operator $\bullet = \circ$
\end{itemize}
