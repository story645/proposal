...
\note{move to artost}
Another use for the sheaf is to provide access to all the records over the space on which the sheaf $\mathcal{O}(\dtotal)$ is defined. The artist \vartist\ could not generate any visualization that needed more information than was in a single record, such as a line plot, if it took as input \dsection, because $\dsection:\dbasepoint \mapsto \delement$ only knows about the record it returns. 
The data bundle \dtotal\ is defined globally on \dbase, but often we want as

 To make these types of visualizations, mathematical objects called sheafs provide a way to construct a \dsection\ on a subset of \dbase. 
...
The sheaf facilitates visualizations such as sliding windows\cite{crouchDynamicGraphsSlidingwindow2013,chuTimeSeriesSegmentation1995} streaming data, or navigation such as pan and zoom\cite{NekrasovskiEvaluationPanZoom2006}.; the jet ensures that the artist only needs a single record on \dbasepoint\ extended with derivatives to render a piece of a graphic. This allows for the creation of highly concurrent and possibly distributed artists. 
