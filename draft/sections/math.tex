...
The image $\vmark(xpos, yposcolor)$ in figure~\ref{fig:artist_heatmap} is a direct lookup into  $\vindex:\gbase\rightarrow\dbase$. Since \dbase\ is 2D continuous space, the indexing variables $(\alpha, \beta)$ define the distance along the space. This is then used by \vindex\ to map into \dbase\ to lookup the color values 
\begin{align}
