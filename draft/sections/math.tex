...
Expressiveness of visual encodings is defined as encoding the relations of the data in the visual space \cite{mackinlayAutomatingDesignGraphical1986} and we formally describe these relations as monoid actions on the data component. We describe relations using monoids because they encompass the Steven's measurement scales \cite{stevensTheoryScalesMeasurement1946,leaFormalizationMeasurementScale}, partial order relations, such as multi-ranked indicators \cite{bruggemannRankingPrioritizationMultiindicator2011}, and are composable \cite{yorgeyMonoidsThemeVariations}. 
...
\begin{align}
    \textbf{associativity}\; & \text{for all } m_j, m_k, m_l \in \monoid \, (m_j \ast m_k) \ast m_l = m_j \ast (m_k \ast m_l)\\
    \textbf{identity}\; & \text{for all } m_j\in \monoid,\,  e\ast m_j = m_j 
\end{align} 
As defined on data components $\dfiber$, a left monoid action \cite{SemigroupAction2021,nlabauthorsAction2021} is a set $\dfiber$ with an action $\bullet: \monoid\times \dfiber \rightarrow \dfiber$ with the properties:
\begin{align}
...
\end{equation}
of each monoid \(\monoid_{i}\) on each component \(\dfiber_{i}\).  The monoid is added to the specification of the fiber \((\fname_i,\, \ftype_i,\, \fttype\, \monoid_i)\)

As defined in \autoref{eq:math:data:monoid:asso}, the functions $m_j, m_k \in \monoid_i$ are composable
\begin{equation}
\begin{tikzcd}
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{In this example, the partially ordered set of weather values are the elements of the fiber component $F_i$. In \autoref{fig:math:data:monoid:total} the function $m_j$ is the identity function $e \in \monoid$ that takes every element $x \in \dfiber_i$ to itself. In \autoref{fig:math:data:monoid:partial}, the function $m_k$ maps the weather elements on the left to the elements on the right such that multiple weather elements on the right are mapped into the same element on the left. The function $m_k$ is order preserving, meaning for example that $sun \geq clouds$ and $m_k(sun) \geq m_k(clouds)$ are both true. The functions $m_j, m_k$ are composable, meaning that whether they are applied in stages, such as the output of $e$ \autoref{fig:math:data:monoid:total} is the input to \autoref{fig:math:data:monoid:partial}, or the functions (the arrows) are first composed $m_j \circ m_k$, the result will be the same.}
    \label{fig:math:data:monoid}
\end{figure}
In \autoref{fig:math:data:monoid}, the arbitrarily chosen functions $m_j, m_k\in \monoid_i$ act on the partially ordered set $\dfiber_i=weather$. In this example, we define the functions $m_j, m_k \in \monoid_i$ applied to elements of  $\dfiber_i$ to be monotone maps \cite{fongInvitationAppliedCategory2019}
\begin{equation*}
    \label{eq:math:data:monoid:monotone}
    if\, a \leq b \; then \; m_j(a) \leq m_j(b) \mid a,b \in \dfiber_{i}
\end{equation*}
In \autoref{fig:math:data:monoid:total}, the function $m_j$ is the identity function $e$ which takes every element in $\dfiber_i$ to itself. The function $m_k$ in \autoref{fig:math:data:monoid:partial} maps the elements of weather on the left to the subset on the right in a way that satisfies the monotonicity condition in \autoref{eq:math:data:monoid:monotone}. For example, on the right $sun \geq clouds$ is true and on the left $m_k(sun) \geq m_k(clouds)$ is true since $m_k(sun)=clouds$ and $m_k(clouds)=rains$ and $clouds\geq rain$. Since the functions $m_j, m_k$ are composable, the result is the same whether the input to $m_k$ is the output of $m_j$ or the functions (arrows) are combined before being applied to the elements in $F_i$. 
...
\end{equation}
between data element \(\delement \in \mathscr{\dtotal}\) and graphic element \(\vartist(\delement) \in \mathscr{\gtotal}\). To be equivariant with respect to monoids action, we propose that an artist carries a monoid homomorphism \(\varphi\)
\begin{equation}
    \varphi: \monoid \rightarrow \monoid^{\prime}
\end{equation}
such that an action in data space \(m \in \monoid\) is equivariant to an action in graphic space \(\varphi(\monoid) \in \monoid^{\prime}\). 
...
We propose that the map from data components to graphic components \(\vchannel: \dsection \mapsto \vsection\) is a monoid \textit{equivariant} map. We conjecture that if the data space or visual space is partially ordered and the map \vchannel\ between the two spaces is equivariant, then it must also be monotonic. By specifying this constraint, we can guarantee that the stage of the artist that transforms data components into graphic representations is equivariant. These constraints then guide the implementation of reusable component transformers \vchannel\ that are composed when generating the graphic. We define the visual transformers \vchannel\ 
\begin{equation}
...
\end{tikzcd}
\end{equation}
such that applying equivariant monoid actions to \(\dtotal_{i}\) and \(\vtotal_{i}\) preserves the map \(\vchannel_{i}: \dtotal_{i}\rightarrow \vtotal_{i}\). In general, the data fiber \(\dfiber_{i}\) cannot be assumed to be of the same type as the visual fiber \(\vfiber_{i}\) and the actions of \(\monoid_i\) on \(\dfiber_{i}\) cannot be assumed to be the same as the actions of \({\monoid_i}^{\prime}\) on \(\vfiber_i\); therefore an equivariant \(\vchannel_i\) must satisfy the constraint  
\begin{equation}
...
without a loss of generality we can assume that an action of\(\monoid_i\) acts on \(\dfiber_{i}\) and on \(\vfiber_{i}\) compatibly such that \(\varphi\) is the identity function. We can make this assumption because we can construct a monoid action of \({monoid_i}^{\prime}\) on \(\vfiber_{i}\) that is compatible with a monoid action of \(monoid_i\) on \(\dfiber_{i}\). We can then compose the monoid actions on the visual fiber \({\monoid_i}^{\prime} \times \vfiber_{i} \rightarrow \vfiber_{i}$ with the homomorphism $\varphi\) that takes \(monoid_i\) to \({\monoid_i}^{\prime}\). This allows us to define a monoid action on \(vfiber_i\) of \(monoid_i\) that is \((m, \velement) \rightarrow \varphi(m)\bullet\velement\), which lets us incorporate \(\varphi\) into the action \(\bullet\) such that \(\varphi\) does not need to be explicitly defined in the constraints. 
...
\end{table}
We generalize equivariance constraints to monoid action equivariance to account for limitations in the types of data that can be described with the Stevens' scales \cite{johnsonPseudoMathematicsMentalSocial1936,thomasMathematizationNotMeasurement2014}. The Stevens measurement types \cite{stevensTheoryScalesMeasurement1946}, listed in \autoref{tab:math:artist:nu}, are specified in terms of groups, which are monoids with invertible operations\cite{remlingAlgebraMath5353}.
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \caption{ The data component \(\dfiber_i\) is the partially ordered set of words describing weather and the visual component \(\vfiber_i\) is a partially ordered set of emojis; the hasse diagrams in the figure are a subset of each fiber component. The \vchannel\ mapping in \autoref{fig:math:artist:nu:valid} from \(\dfiber_i\) to \(\vfiber_i\) is monotonic, and therefore equivariant. One example of this monotonicity is that \(rain \geq storm \) and \(\nu(rain) \geq \nu(storm)\). In contrast, the map from data component to visual component in \autoref{fig:math:artist:nu:invalid} is not monotonic, and therefore not monoid equivariant, because \(rain \geq storm\) is mapped to elements with the reverse ordering such that \(\nu(rain) \geq \nu(storm)\) is false.}
  \label{fig:math:artist:nu}
\end{figure}
\autoref{fig:math:artist:nu} illustrates the encoding of weather data as umbrella emojis. The weather data is a subset of the partially ordered data component \(\dfiber_i\), while the umbrellas are a subset of the partially ordered visual component \(vfiber_i\). In \autoref{fig:math:artist:nu}, \vchannel\ is equivariant and therefore monotonic, meaning it maps elements from \(\dfiber\) to elements in \(\vfiber_i\) such that the monotonicity condition in \autoref{tab:math:artist:nu} is satisfied. In contrast, the \vchannel\ in \autoref{fig:math:artist:nu:invalid} is not equivariant and therefore not monotonic, which also means it is invalid. To satisfy the monotonic condition for \(rain \geq storm\), either the arrow between \textit{storm} and \textit{rain} or the arrow between the \textit{closed} and \textit{wet} umbrellas in \autoref{fig:math:artist:nu:invalid} would have to go in a different direction. This is because the monotonic condition \(rain \geq storm \implies \vchannel(rain) \geq  \vchannel(storm)\) is not met since \(\vchannel(rain) \leq\vchannel(storm)\). 


\subsubsection{Visualization Assembly \vmark}
...
\subsubsection{Case Studies: Scatter, Line, Image}
Given the continuities described in \ref{fig:math:graphic:retraction:map}, 
we illustrate a minimal \vmark (\vmarkd) that will generate the most minimal visualizations associated with those continuities: non-overlapping scatter points, a non-infinitely thin line, and an image. 
\begin{figure}[H]
