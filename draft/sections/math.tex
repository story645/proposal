...
\label{sec:data}
Lets say we are working with the Global Historical Climatology Network\cite{AnOverviewoftheGlobalHistoricalClimatologyNetworkDailyDatabase}, which is a dataset of global daily meterological observations. Each record includes the timestamp, location, and measurements such as temperature, precipitation, and snowfall. Using a fiber bundle allows us to express the properties of the variables(\ref{sec:data_fiber}), the connectivity of the observations (\ref{sec:data_base}), and tables of records (\ref{sec:data_section}).

\subsubsection{Variable Field in F }
\label{sec:data_fiber}
The fiber is a topological space containing the set of all observations that could possible be in the dataset, even values that do not actually exist in the dataset. For example, lets construct a fiber bundle that is just the temperature column from GHCN. Then the fiber $F$ is the range of physically possible temperature values $\text{Temp} \subset \mathbb{R}$. Lets rename this
\begin{equation}
F_0 = \{\text{Temp}\}
\end{equation}
so that more variables can be added to the fiber. For example, lets add the time from GHCN $F_1 =\text{Time}$
\begin{equation}
F =  \{\text{Temp}\} \times \text{Time}
\end{equation}

The elements in a given fiber set do not need to be scalers; for example the location can be encoded as $F_3=\text{Loc}$ where $(\text{lat},\text{lon{}) \in \text{Loc}$.  We add the location to the fiber by adding the set to the cartesian product
\begin{equation}
    F =  \{\text{Temp}\} \times \text{Time} \times \text{Loc}
\end{equation}

We also add the variable name and measurement scale $M$ monoid to the fiber $F_i$ such that 
\begin{align}
    F_0 &= (\mathrm{temperature}, \text{Temp}, M_0)\\
    F_1 &= (\mathrm{time}, \text{Time}, M_1)\\
    F_2 &= (\mathrm{location}, \text{Loc}, M_2)
\end{align}
 or more generally (name, set, monoid)
 \begin{equation}
    F_i = (\mathrm{name}, \{x_{i}, \ldots, \x_{n}\}, M_{i})
 \end{equation}
...
\end{equation}
while also retaining the individual sets $F_{i}$ that we will transform into visual characteristics of the graphic. 
...
As mentioned in section~\ref{sec:intro_data_structure}, datasets have a semantic topology wherein the values are interpreted as discrete observations or part of a timeseries or map, or nodes in a networks \cite{munznerWhatDataAbstraction2014,geveci2012vtk}. one way of encoding this underlying structure are the $K$s illustrated in figure~\ref{fig:base_space_div}, $K$ does not directly know the values; instead it is the sections that define the lookup between keys $k \in K$ and the corresponding values in $E$.
