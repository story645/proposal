...
The section \dsection\ is defined globally on \dbase, including infinite \dbase\ for streaming data and total \dbase\ for chunked data, but often we want as input into the artist \vartist\ a subset of the data. For example sliding window\cite{crouchDynamicGraphsSlidingwindow2013,chuTimeSeriesSegmentation1995} based visualizations of streaming data, or visualizations that support pan and zoom navigation\cite{nekaro}

To support working with a subset of data, we can take the sheaf $\mathcal{O}(\dtotal)$ as input into the artist.

The sheaf $O(\dtotal)$ is the localized section of fibers $\iota^*\dsection: U \rightarrow \iota^*\dtotal$

\begin{equation}
    \label{eq:sheaf}
    \begin{tikzcd}
        \iota^*\dtotal \arrow[d, "\pi"']           & \dtotal \arrow[d, "\pi"'] \arrow[r, "\iota^*"']         \\
        U \arrow[u, "\iota^*\dsection"', bend right] & \dbase \arrow[u, "\dsection"', bend right] \arrow[r, "\iota"']
    \end{tikzcd}
\end{equation}
