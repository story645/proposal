...
\begin{figure}[ht]
    ...
    %% simpler still replace w/ timeseries of temp, 1D fiber
\end{figure}
...
As illustrated by figure~\ref{fig:mobius}, the vertical lines $F$ are the range of possible values embedded in the total space $E$. The topol $K$ is a representation of the connectivity of the points in $E$. The function $\pi$ is the mapping from a point on a specific fiber $F_{k}|k\in K$ in $E$ to a location $k \in K$. 
...
\end{figure}
%%% might 
The fibers encode the set of all possible values each observation can take. For example, the fiber bundle in figure~\ref{fig:movie} is the set of all possible movies, where each $k \in K$ is a frame in the movie. Each \sigma 
