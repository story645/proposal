...
As discussed in the introduction, visualization is generally thought of as structure preserving maps from data into graphic representation. In order to formalize this statement, we capture the connectivity of the records with topolology and the structure on the components of the records with the monoidal actions on the sets those values belong to. 

In this section we formally define that structure and how it is preserved via equivariant maps tha can express discrete and continuous topology. 

We can then specify that a faithful visual mapping is structure preserving, and apply these constraints to visualizations we may want to develop or implement. 
...
%%% so we can do units, static + dynamic type checking, so that we can tak about variables 
...
%% pin down the structure we're trying to preserve, this is what we mean by equivarance, most general useful formalism is a monoid, most specifically, more commonly it's a group but sometimes it's a mpnoid
...
\label{sec:data_base}
% scatter and line, heatmap express the continuity of the data, deeply implicit in every visualization system but equivariance of continuity of the records, explicit  topplogy/structure preserving - contuinity is the topology   
