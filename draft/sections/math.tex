...
\end{description}
where \dtotal\ is the space of all possible datasets with these components and continuity. The fiber bundles mentioned in this work are assumed to be \note{locally} trivial\cite{spanier1989algebraic,LocallyTrivialFibre}, unless otherwise specified. We assume \note{local} triviality to dispenses with extra structure on $\dtotal$ and focus on the fiber and base space. 
\note{(and can do so b/c all bundles are locally trivial)}
