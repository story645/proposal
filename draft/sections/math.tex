...
\begin{table}[ht]
    ...
    color                       & color, linecolor, facecolor, markerfacecolor, edgecolor  & $\mathbb{R}^{4}$ \\ \hline
...
\end{tikzcd}
\end{equation}
where the diagram commutes such that  $\nu_{i}\circ f(\tau_{i}) = f \circ \nu_{i}(\tau_{i})$. A monoid is a set $M$ with a closed associative binary operation ($\*$) and an identity element $e$. 
