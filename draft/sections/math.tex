...
\end{equation}
 ...
 is mapping from a region $s$ on a mathematical encoding of the image to a region $xy$ on the screen that the renderer then maps to pixel space.
 
 \subsubsection{Section $\rho$}
 For a physical screen display, the predisplay space is a trivial fiber bundle $H=\mathbb{R}^{7}\times S$ such that $\rho$ is
\begin{equation}
    \rho(s)  = \{x, y, z, r, g, b, a\}
    \label{eq:rho}
\end{equation}
...
with fiber $P$ that encodes visual paramters such as position or color. As in section~\ref{sec:data_fiber}, the fields in $P$ are strongly typed. Inside the artist is also the equivalance class map $\xi: S \rightarrow K$ that goes from a region $s$ to its associated point $k$
\begin{equation}
    \begin{tikzcd}
        ...
        \end{tikzcd}
\end{equation}
such that when $\xi(s) = k$, $\xi^*\tau(s) = \tau(k)$.
...
such that the visual section $\mu: K \rightarrow V$  is equivalent to the visual transformation of the data $\mu(k) = \nu\circ\tau(k)$
\begin{equation}
    \begin{tikzcd}
        E \arrow[r, "\nu"]                      & V \\
...
The constraint on $\nu: \tau\rightarrow \mu$ is that $\tau$ and $\mu$ must be equivariant. 

%%set up proofs here



\subsubsection{Visual Composition}
