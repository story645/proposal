...
We encode indexing from $K$ to $E$ as sections $\tau$ of the fiber bundle that return corresponding records. For example when $K$ has
\begin{align*}
...
which is a decomposition of $F$. The $K$ remains the same because the connectivity of records does not change just because there are fewer variables in each record.
