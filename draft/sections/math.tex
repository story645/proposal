...
As described in equation~\ref{eq:local_trivial}, there is a local space $U \subset K$ around every $k$. The inclusion map $\iota: U \rightarrow K$ can be pulled back such that $\iota^{*}E$ is the space of $E$ restricted over $U$. The localized section of fibers $\iota^*\tau: U \rightarrow \iota^*E$ is the sheaf $O(E)$. The neighborhood of points the sheaf lies over is the stalk $\mathscr{F}_k$ \cite{StalkSheaf2019,spanier1989algebraic}
...
The visualization specification function $\Xi=Q\circ\nu$  builds $\rho$ functions via a function $Q$ that compose $\nu_{param}$ visual characteristic mappers
\begin{equation}
    \nu_{param}(\tau_{param}) = \mu_{param}
\end{equation}
where $\tau_{param}$ is a field in $\tau$ bound to the paremeter and $\mu_param$ is an intermediate visual representation such that $\nu_shape(\tau_shape)$ returns a $\mu_shape$ function that returns a shape for each $k$. $Q$ then composites the $\nu$ functions to build $\rho$ as described in equation~\ref{eq:rho}

\begin{equation}
    Q(\mu_{param_0}, \ldots, \mu_{param_n}(s) = \rho(s)
    \label{eq:Q}
\end{equation}

$Q$ is analogous to a visualization type such as scatter or line plot, and $\nu$ to aesthetic parameters such as line color or marker shape \cite{bertinIIPropertiesGraphic2011, munznerMarksChannels} and composites the $\nu$ mappings into $\rho$ functions as 
