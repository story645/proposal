...
The function $rho: S \rightarrow H$ is composed of subfunctions that are dependent on the prerender fiber $D$. In the case of a screen
\begin{equation}
...
where $(\alpha, \beta)$ are coordinates on $S$. The function $Q$

\begin{equation}
    \begin{tikzcd}
        \Gamma(\theta) \arrow[rr, "{Q(\nu_{0}(c),\ldots,\nu_n(c))}"] &  & {\rho_X, \rho_Y, \rho_Z, \rho_R, \rho_B, \rho_G, \rho_A}
    \end{tikzcd}
\end{equation}
