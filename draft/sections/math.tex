...
The measurement spaces $X$ are each variables and have the properties of measurement scales, such as Steven's nominal, ordinal, interval, and ratio \cite{stevensTheoryScalesMeasurement1946}. 
Spivak provides a formal way of attaching a name to each $X \in F$ and mapping from the value back to the variable type by formulating $E$ in terms of databases \cite{spivakSIMPLICIALDATABASES}. He defines a total space $U$, analogous to $E$, that is the disjoint union of all possible elements of all possible types 



For example, the interval $[T_{min}, T_{max}]$ is a restriction on $\mathbb{R}$ that is part of the definition of a type named $\mathrm{Temperature}$ and  $[t_{min}, t_{max}]$ is a restriction on $\mathbb{R}$ that is part of a definition of a type named $\mathrm{Time}$. As in the case of $E$ embedding the $F$ on which these values live, $U$ contains all pairs of all values of these types $U \coloneqq (\mathrm{Temperature}, \mathrm{Time})$. Spivak defines U as being over the space of $\mathrm{DT}$, which is the space of data types $\mathrm{DT} = \{\mathrm{Temperature}, \mathrm{Time}}$. 
