...
\begin{equation}
     \rho(u, v) = Q(\nu_{param_{n}}(\tau_{c_0}), \ldots \nu_{param_{n}}(\tau_{c_n}))(u,v)
    \label{eq:Q}
\end{equation}

$Q$ composites $\nu$ functions because multiple parameters can target the same field in $D$, for example $\nu_{ypos}$ and $\nu_{linewidth}$ together determine the output of the $\rho_y$ y position of a line on screen. 
...
For the aesthetic mapping function $\nu$ to faithfully transform the data in $\xi^*\tau$, the $\nu$ and $\tau$ must be symmetrical under all transformations such that
\begin{equation}
    \nu_{param}(\tau_{})
\end{equation}
