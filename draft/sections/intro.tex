...
This work aims to develop a model of visualization such that a tool built using this model could support visualizations as varied as those of Du Bois in figure~\ref{fig:}; to do so, we first discuss the criteria by which a visualization is evaluated. 


Byrne et al. describe visualizations as having graphic and figurative components; graphic representations are maps from data values to visual encodings\cite{bertinSemiologyGraphicsDiagrams2011a}, while figurative representations have meaning due to their similarity in shape to external concepts \cite{byrneAcquiredCodesMeaning2016}. In figure~\ref{fig:intro_dbc}, Du Bois combines a graphical representation where glyph size varies by population with a figurative representation of those glyphs as the countries the data is from, which means that the semantic nominal and numerical properties of the data are preserved in the graph. Norman states that when the properties of the representation match the properties of the information being represented\cite{norman_things_smart, NaturalnessPrincipleInfoVis} then the visualizations are more understandable 
, but does not give criteria for what it means for the properties to match. Tufte specifies that visual representations must be in proportion to the quantitative data being represented, but otherwise his notion of graphic integrity is heavily context dependent\cite{tufteVisualDisplayQuantitative2001}. 
