...
\begin{frame}{Model data as a fiber bundle \cite{butlerVectorBundleClassesForm1992,butlerVisualizationModelBased1989}}
    ...
    \begin{description}
        ...
        \item[fiber space \dfiber] variable characteristics
...
\begin{frame}{Encode variable types in a schema like fiber \cite{spivakDatabasesAreCategories2010,spivakSIMPLICIALDATABASES}}
    \begin{figure}[H]
        \centering
        \includegraphics[height=.3\textwidth]{figures/math/fiber.png}
        \label{fig:data_fiber_example}
    \end{figure}
    \begin{block}{Monoid actions define the structure on $\dfiber_i$}
        \begin{equation}
            \dfiber = \dfiber_0 \times \ldots \times \dfiber_i \times \ldots \times \dfiber_n
        \end{equation}
    
        $\monoid_{i}$ actions (e.g. rotation, partial ordering) 
        \begin{equation}
            \bullet: \monoid_i\times \dfiber_i \rightarrow \dfiber_i
        \end{equation}
        must be associative and have an identity element. 
...
\begin{frame}{Structural \textit{keys} with associated \textit{values} \cite{munznerVisualizationAnalysisDesign2014}}
    ...
    \begin{block}{\dbase\ is an indexing space independent of component semantics}
        \begin{equation}
...
\begin{frame}{Graphic Bundle $(\gtotal,\,\gbase,\,\pi ,\,\gfiber)$}
    Continuity is preserved via the deformation retraction $\vindex: \gbase\rightarrow\dbase$
    \begin{equation}
        \begin{tikzcd}[ampersand replacement=\&]
...
\end{equation} 
and is implemented via an intermediate visual bundle $(\vtotal,\,\dbase,\,\pi ,\,\vfiber)$
\begin{equation}
...
\begin{frame}{Existing models don't have continuity, many evaluative}
        \begin{description}
            \item[algebraic process] data and viz transforms are symmetric \cite{kindlmannAlgebraicProcessVisualization2014}
            \begin{equation*}
                \begin{tikzcd}[ampersand replacement=\&]
                    \dtotal \arrow[d, "m"'] \arrow[r, "r_1"] \& \vtotal \arrow[r, "\nu"]  \& \gtotal \arrow[d, "\varphi(m)"] \\
...
\begin{frame}{Equivariance: Partial Order \& and Stevens' Scales \cite{stevensTheoryScalesMeasurement1946}}
    \begin{figure}
...
\begin{frame}{TEAM driven rearchitecture of Matplotlib}
    \begin{itemize}
        \item fiber bundle with schema is a structure rich abstraction
        \begin{itemize}
