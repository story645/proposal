...
\end{table}
The equivariance constraints on the Stevens measurement scales \cite{stevensTheoryScalesMeasurement1946} are listed in \autoref{tab:math:artist:nu}. These are derived from the group actions on the scales, as defined by Stevens.

\begin{figure}[htb]
  \centering
  \includegraphics[width=1\columnwidth]{monoid_equivariant.png}
  \caption{In the top two panels \vchannel\ maps to visual elements such that the relative order of the visual elements is the same as the data. In the bottom right panel, \textit{rain} is mapped to a visual element that is less than the visual element \textit{storm} is mapped to, which is not an allowed \vchannel. Changing the map so that \textit{storm} goes to the same visual element as \textit{rain} yields an order preserving map and is therefore a valid \vchannel.}
  \label{fig:math:artist:nu}
\end{figure}

The equivariance constraint for monoid actions is that relative order is preserved, as shown in the top two panels of \autoref{fig:math:artist:nu}. In the bottom right panel the ordering of \textit{rain} and \textit{storm} is swapped in the visual elements; in the bottom left panel, \textit{storm} is instead mapped to the same visual element as \textit{rain} so that the ordering is preserved. 
